...
public class Field {
    ...
    private void loopGeneration(double percentSizeOfLoop){
        ...
        if((rowCount-1)/2==2 && (columnCount-1)/2==2 && counter==3 ){throw new RuntimeException("Impossible case");}
    ...
    private void numbersGeneration( double percentOfClues) {
        for(int y = 1; y<rowCount; y+=2) {
            for (int x = 1; x < columnCount; x+=2) {
                ...
                ((Clue) elements[y][x]).setClueState(ClueState.VISIBLE);
            }
        }
        int rowCountOpt = (rowCount-1)/2;
        int columnCountOpt = (columnCount-1)/2;
        if(percentOfClues>=1 || percentOfClues<=0){throw new RuntimeException("Invalid percent of clues");}
        Random random = new Random();
        int counter = (int) Math.round(rowCountOpt*columnCountOpt*percentOfClues);
